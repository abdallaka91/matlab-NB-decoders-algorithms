...
clear
pth6 = (fullfile(pwd, 'results/'));
a = load(fullfile(pth6, '204.102.3.6.16_2024_06_04_13_48_46.mat'));
b = load(fullfile(pth6, '204.102.3.6.16_2024_06_03_16_52_09.mat'));
c = load(fullfile(pth6, '204.102.3.6.16_2024_06_04_10_49_20.mat'));
...
xlim([0 6])
ylim([1e-7 1e-0])
legend({'16 iters - 1 attempt', '96 iters - 1 attempt', '16 iters - 6 attempts', 'EMS (from the paper)'})
% legend({'MV-SF | 20 iters | 1 attempt', 'MV-SF | 20 iters | 4 attempts', 'EMS (from the paper)'})
xlabel('EbNo (dB)')
ylabel('BER')
% position = [0.1 + 0.4 * 8 / 100, 0.8 - 0.8 * 50 / 100, 0.8, 0.1];
