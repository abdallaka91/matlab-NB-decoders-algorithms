...
parf = 80;
ebn0              = 3.4:0.2:4.4;  % SNR values to simulate
snr_cnt           = length(ebn0);
max_gen           = 1e6;
...
max_max_iter      = 3000; % keep try above T with single VN permute until l = max_max_l
syndrome_weight   = 0.95;   % Syndrome weight parameter
theta             = -[2.9 2.6 2.6];   % Flipping threshold
mnk               = [15 15 15];
regns             = [80 150];
...
conf_detail.a15= sprintf("max bits to flip per iter : %d, %d, %d",mnk);
conf_detail.a16= sprintf("Binary syndrom weight : %.3f",syndrome_weight);
conf_detail.a17= sprintf("Flipping threshold : %.3f, %.3f, %.3f",theta);
conf_detail.a18= sprintf("Flipping thresold regions : %d, %d",regns);
conf_detail.a19= sprintf("perturbation noise scaling : %.3f",eta);
conf_detail.a20= sprintf("max seq generation : %e", max_gen);
conf_detail.a21= sprintf("single VN symbol flipping max Hamming distance 1: %d", nones1);
conf_detail.a22= sprintf("single VN symbol flipping max Hamming distance 2: %d", nones2);
conf_detail.a23= sprintf("single VN symbol flipping applying threshold: %d", sngl_VN_th);
...
for i0=1 : snr_cnt
    ...
    while FER_cnt(i0) < max_err_cnt && F_gen(i0)<=max_gen
       ...
       parfor pp=1 :parf
            ...
            [~, failed__, ~, ~, E__, ~, seqgf__, iters__, succ_ref, i111(pp)] = decodeGDBFvecNB(p,pw, h_arr,str_cn_vn,dc,dv,...
                mul_mat, add_mat, y,hl, h, N, max_iter, syndrome_weight, theta,mnk,regns, nsigma,dev_hamm1, dev_hamm2, sngl_VN_th, max_max_iter);
            
            if succ_ref
                succ_ref_cnt = succ_ref_cnt  +1;
            end
