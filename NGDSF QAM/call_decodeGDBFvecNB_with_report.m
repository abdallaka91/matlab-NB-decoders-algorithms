...
parf = 20;
ebn0              = 7.5;  % SNR values to simulate
snr_cnt           = length(ebn0);
max_gen           = 1e6;
max_err_cnt1      = 10; % at low Eb_No(<Eb_No_thrshld)
max_err_cnt2      = 10; %at high Eb_No
Eb_No_thrshld     = 4.8;
mnk               = 25;
max_iter          = 1500;    % Max iterations for decoding
max_max_iter      = 2000; % keep try above T with single VN permute until l = max_max_l
syndrome_weight   = 0.85;   % Syndrome weight parameter
theta             = 0.2;   % Flipping threshold
...
report_fle_nme0 = strcat('Non_Binary_QAM_GDSF_', H_matrix_mat_fl_nm, '_dated_' ,current_date ,'_' ,current_time);
...
for i0=1 : snr_cnt
    ...
    while FER_cnt(i0) < max_err_cnt
        ...
        for pp=1 :parf
            ...
            [HD2, HD2_cmp] = HD_QAM(y_cmp, norm_const);

            % load yy.mat
            
%             [failed_mx, seqgf, l]  = decodeGDSFvecNB_QAM(h_arr,mul_mat, add_mat,...
%     y,hl, h, T, w, theta,nsigma,  cnstl1)
            [failed__, seqgf__,seqgf_cmplx__, iters__] = decodeGDSFvecNB_QAM(...
                h_arr,mul_mat, add_mat,y_cmp,hl, h, max_iter,max_max_iter, str_vn_cn, str_cn_vn,...
                dc, dv, syndrome_weight,theta,nsigma,mnk,...
                norm_const, code_seq);
