...
parf = 4;
ebn0              = 4.2:0.01:4.22;  % SNR values to simulate
snr_cnt           = length(ebn0);
max_gen           = 1e6;
...
syndrome_weight   = .92;   % Syndrome weight parameter
theta             = -2.35;   % Flipping threshold
eta               = 1.0;     % Perturbation noise scale parameter
nones1            = 4; %max hamming dist
nones2            = 1; %max hamming dist
sngl_VN_th = 4.5; %dB above whichSNR check if a single symbol flipping could reduce nb of faild by its dv
...
conf_detail.a17= sprintf("perturbation noise threshold : %.3f",eta);
conf_detail.a18= sprintf("max seq generation : %e", max_gen);
conf_detail.a19= sprintf("single VN symbol flipping max Hamming distance 1: %d", nones1);
conf_detail.a20= sprintf("single VN symbol flipping max Hamming distance 2: %d", nones2);
conf_detail.a21= sprintf("single VN symbol flipping applying threshold: %d", sngl_VN_th);
