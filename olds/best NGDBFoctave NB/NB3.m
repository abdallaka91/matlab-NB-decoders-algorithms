clear
aaa=load('102_204_GF16_first_triL.mat');
...
parf = 160;
Ebnos           = 3.75;  % SNR values to simulate
snr_cnt         = length(Ebnos);
max_gen           = 1e6;
F                 = 500;      % Number of frame errors to observe
...
for sdx=1:snr_cnt
    ...
    while (keepGoing)
        ...
        parfor pp=1 :parf
            ...
            [~, failed__, ~, ~, E__, ~, seqgf__, iters__] = decodeGDBFvecNB3(p,pw, h_arr,dc,dv, mul_mat, add_mat, y,hl, h, N, T, w, theta,mnk, nsigma);
