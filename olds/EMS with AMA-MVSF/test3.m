...
parforN = 50;
attemt_iters = [16*ones(1,12)];
max_attempt = length(attemt_iters);
% di{1} = [0 0];
% di{2} = [0 1];
% di{3} = [0 2 1];
...
% conf_detail.a14iter = sprintf("iters : %s", strjoin(string(attempt_iters), ' | '));
conf_detail.a15max_attempt = sprintf("max nb of max_attempts : %d",max_attempt);
...
for i0 = 1 : snr_cnt
    ...
    while FER(i0) < max_err_cnt && gen_seq_cnt(i0)<max_gen
        parfor j = 1 : parforN
            ...
            LLR_out=-LLR_2;
            if wth_ems
                [~, dec_seq, success_dec,~,LLR_out] = EMS2(...
                    -LLR_2, 1, mul_mat, add_mat, div_mat, h,str_cn_vn, dc,...
                    str_vn_cn, dv, nmems, ncems, c2v_comp_fact, comp_ECN);
                [~,HD2]=min(LLR_out,[],2);
                HD2 = HD2'-1;
            ...
            if ~success_dec
                [iters, dec_seq, success_dec] = ...
                    presorted_MVSF_with_rinfrc(-0.9*LLR_out,attemt_iters, mul_mat, add_mat, div_mat,...
                    h,str_cn_vn, dc, dev_lsts, nm, v_weights, max_attempt);
                needed_iters_(j) = iters+1;
            else
                iters=1;
            end
