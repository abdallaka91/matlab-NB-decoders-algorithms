...
Ebnos           = 4.0;  % SNR values to simulate
...
for sdx=1:snr_cnt
    ...
    while (keepGoing)
        ...
        dgf = HD_bin_gf(sign(y),p,N,pw);
        nerrS_decs = sum(dgf~=code_seq);
        Ser_decs = nerrS_decs/N;
        [d, failed, Sgf, Sb, E, seqb, seqgf, iters] = decodeGDBFvecNB2(p, str_cn_vn,dc,mul_mat, add_mat, y,hl, h, N, T, w, theta, nsigma);
