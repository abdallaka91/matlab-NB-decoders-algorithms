...
snr_cnt           = length(ebn0);
max_gen           = 1e6;
max_err_cnt1      = 20; % catch max_err_cnt1 error frames at low Eb_No(<Eb_No_thrshld)
max_err_cnt2      = 20; % catch max_err_cnt1 error frames at high Eb_No
Eb_No_thrshld     = 0; % low-high Eb_No threshold 
mnk               = 15; % flip the mnk symbols that have the lowest flipping metric 
max_iter          = 1000;    % Max iterations for decoding
max_max_iter      = 1350; % keep try above max_iter with single VN permute until l = max_max_l
...
for i0=1 : snr_cnt
    ...
    msg = sprintf("SNR = %.3f dB, FER = %d/%d = %.8f,// BER = %d/%d = %.8f, aver_iter = %.3f, max_ndd_iter = %d\n",...
        Snr_db(i0), FER_cnt(i0), F_gen(i0), FERR(i0), BER_cnt(i0),B_gen(i0),...
        BERR(i0), aver_iter(i0), 0 );
    fprintf(msg)
    ...
    while FER_cnt(i0) < max_err_cnt
        ...
        for pp=1 :parf
            ...
            nf1 = sum(HD2~=code_seq);
            
            dec_cnt = 1;
            failed___=zeros(dec_cnt,1);
            iters___ = failed___;
            seqgf___ = zeros(N,dec_cnt);
            
            for dec_i=1:dec_cnt
            [failed___(dec_i), seqgf___(:,dec_i), iters___(dec_i)] = decodeGDSFvecNB_QAM(...
            ...
            [mn_iter_ndd,mn] = min(iters___);
            if mn_iter_ndd>mx_ndd_iter
                mx_ndd_iter =mn_iter_ndd;
            end
            failed__= failed___(mn); seqgf__ = seqgf___(:,mn); iters__ = iters___(mn);
        ...
        msg = sprintf("SNR = %.3f dB, FER = %3d/%8d = %.3e,// BER = %4d/%11d = %.3e, aver_iter = %.3f, max_ndd_iter = %d\n",...
            Snr_db(i0), FER_cnt(i0), F_gen(i0), FERR(i0), BER_cnt(i0),B_gen(i0),...
            BERR(i0), aver_iter(i0),mx_ndd_iter );
        fprintf(msg)
