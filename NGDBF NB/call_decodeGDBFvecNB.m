...
parf = 200;
Ebnos           = 4.2:0.01:4.22;  % SNR values to simulate
snr_cnt         = length(Ebnos);
max_gen           = 1e6;
F                 = 40;      % Number of frame errors to observe
mnk               = 30;
R                 = K/N;  % code rate
T                 = 1000;    % Max iterations for decoding
max_max_l         = 2000; % keep try above T with single VN permute until l = max_max_l
w                 = .92;   % Syndrome weight parameter
...
statstc_iter = zeros(snr_cnt,max_max_l);
    ...
    while (keepGoing)
        ...
        parfor pp=1 :parf
            ...
            [~, failed__, ~, ~, E__, ~, seqgf__, iters__] = decodeGDBFvecNB(p,pw, h_arr,str_cn_vn,dc,dv,...
                mul_mat, add_mat, y,hl, h, N, T, w, theta,mnk,nsigma,dev_hamm1, dev_hamm2, sngl_VN_th, max_max_l);
        ...
        for pp =1 : parf
            statstc_iter(sdx, statstc_iter__(pp)) = statstc_iter(sdx, statstc_iter__(pp)) + 1;
        end
