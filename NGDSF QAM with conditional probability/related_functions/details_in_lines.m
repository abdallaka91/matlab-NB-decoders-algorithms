function text_lines  = details_in_lines(Snr_db, FER,BER, SER, gen_seq_cnt,K,p, aver_iter, conf_detail, file_name, save_rslt)

snr_cnt = length(Snr_db);
msg='\n';
for i0=1:snr_cnt
    formatSpec = msg + "%s";
    msg0 =  sprintf("SNR = %.3f dB, FER = %4d/%8d = %.3e || SER = %4d/%9d = %.3e || BER = %5d/%11d = %.3e, aver_iter = %.3f\n" +...
        "------------------------------------------------------------------------------------------------------------------------------------------------\n",...
        Snr_db(i0), ...
