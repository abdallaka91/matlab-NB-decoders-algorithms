    ...
    for i = 1 : M
        ...
        for l = 1 : L
            ...
            for addr = 1 : dc(i)
                ...
                % if l==1
                %     if addr>dc(i)-Dev_pos_cnt && msg_gf_i2j0(addr,1)== Mv2c_GF_i(addr,1)
                %         msg_llr_i2j0(addr,1)= v_weights(2);
                %     else
                        % msg_llr_i2j0(addr,1)=v_weights(1);
                %     end
                % end
                % if msg_gf_i2j0(addr,l)== Mv2c_GF_i(addr,1)
                %     msg_llr_i2j0(addr,l) = v_weights(1);
                % elseif sum(msg_gf_i2j0(addr,l)== Mv2c_GF_i(addr,:))==1
                %     msg_llr_i2j0(addr,l) = v_weights(2);
                % else
