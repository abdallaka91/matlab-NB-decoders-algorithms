...
syndrome_weight   = 1.66;   % Syndrome weight parameter
theta             = -1.35;   % Flipping threshold
eta               = 1.3;     % Perturbation noise scale parameter
...
bb=0;
load catched_err_7p2db.mat
parf = size(catched_err,1);
...
while 1
    for pp=1 :parf
        ...
        code_seq = catched_err_seq(pp, :);
        info_seq = code_seq(1:K);

        code_seq_comp = norm_const(code_seq+1);

        y_cmp = catched_err(pp,:);
        ...
        if failed__~=0
            ...
            [nerrS_decd__, nerrB_decd__] = SER_BER(code_seq,seqgf__,p, 0, 0);
