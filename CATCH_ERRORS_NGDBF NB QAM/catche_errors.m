    ...
    while FER_cnt(i0) < max_err_cnt
        ...
        for pp=1 :parf
            ...
            [~, failed__, ~, ~, E__, ~, seqgf__, iters__] = decodeGDBFvecNB(p,pw, h_arr,str_cn_vn,dc,dv,...
                ...
                , dev_hamm2, sngl_VN, max_max_iter,gray_labels, code_seq);
        ...
        catched_err_seq_i = catched_err_seq_i(iser,:);

        if cc>0
            if bb+cc<=nn
            catched_err(bb+1:bb+cc, :) = catched_err_i;
            catched_err_seq(bb+1:bb+cc, :) = catched_err_seq_i;
            else
                gh = nn-bb;
                catched_err(bb+1:bb+gh, :) = catched_err_i(1:gh,:);
                catched_err_seq(bb+1:bb+gh, :) = catched_err_seq_i(1:gh,:);
            end
        end
        bb = bb+cc;
        % if bb>nn
