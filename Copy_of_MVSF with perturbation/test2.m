...
parfor jjj = 1 : 1000
    ...
    LLR_2 = round(LLR_2);
    [iters, dec_seq, success_dec, LLR_3] = presorted_MVSF_4(LLR_2, max_iter, mul_mat, add_mat, div_mat, h,str_cn_vn, dc, ...
    ...
    if success_dec
        ...
        for j = 1 : N
            if ~s1(j)
                b = LLR_2(j,:);
                [~,i1] = max(b);
                bb=LLR_3(j,dec_seq(j)+1);
                LLR_3(j,dec_seq(j)+1) = 0;
                LLR_3(j,i1) = bb;
            end
        end

        [iters1, dec_seq1, success_dec1, LLR_4] = presorted_MVSF_4(LLR_3, max_iter, mul_mat, add_mat, div_mat, h,str_cn_vn, dc, ...
            dev_lsts,dev_pos,nm,v_weights, Dev_pos_cnt, PERM_rng, PERM, max_trial);

        if success_dec1
            cnt = cnt + 1;
        else
            synd=decod_prod(dec_seq1, h, str_cn_vn, mul_mat, add_mat);
            h1 = 0*h;
            for i = 1 : M
                if synd(i)
                    h1(i, str_cn_vn{i}) = 1;
            ...
            for j = 1 : N
                if ~s1(j)
                    b = LLR_3(j,:);
                    [~,i1] = max(b);
                    bb=LLR_4(j,dec_seq1(j)+1);
                    LLR_4(j,dec_seq1(j)+1) = 0;
                    LLR_4(j,i1) = bb;
                end
            end

            [iters2, dec_seq2, success_dec2, LLR_5] = presorted_MVSF_4(LLR_4, max_iter, mul_mat, add_mat, div_mat, h,str_cn_vn, dc, ...
                dev_lsts,dev_pos,nm,v_weights, Dev_pos_cnt, PERM_rng, PERM, max_trial);

            if success_dec2
                cnt = cnt + 1;
            end
            %                 synd=decod_prod(dec_seq2, h, str_cn_vn, mul_mat, add_mat);
            %                 h1 = 0*h;
            %                 for i = 1 : M
            %                     if synd(i)
            %                         h1(i, str_cn_vn{i}) = 1;
            ...
            %                         b = LLR_4(j,:);
            %                         [~,i1] = max(b);
            %                         bb=LLR_5(j,dec_seq2(j)+1);
            %                         LLR_5(j,dec_seq2(j)+1) = 0;
            %                         LLR_5(j,i1) = bb;
            %                     end
            %                 end
            %
            %                 [iters3, dec_seq3, success_dec3, LLR_6] = presorted_MVSF_4(LLR_5, max_iter, mul_mat, add_mat, div_mat, h,str_cn_vn, dc, ...
            %                     dev_lsts,dev_pos,nm,v_weights, Dev_pos_cnt, PERM_rng, PERM, max_trial);
            %
            %                 if success_dec3
            %                     cnt = cnt + 1;
