...
di{2} = [0 1];
di{3} = [0 2 1];
v_weights = [0.8  0.3]*256*4; %good forDV=3
parforN = 50;
max_attempt = 1;
% di{1} = [0 0];
% di{2} = [0 1];
...
for i0 = 1 : snr_cnt
    ...
    while FER(i0) < max_err_cnt && gen_seq_cnt(i0)<max_gen
        parfor j = 1 : parforN
            ...
            [~,HD1] = max(LLR_2,[], 2);
            HD1 = HD1'-1;
            [iters1, dec_seq1, success_dec1,~,LLR_out] = EMS2(...
                -LLR_2, max_iter_EMS, mul_mat, add_mat, div_mat, h,str_cn_vn, dc,str_vn_cn, dv, nm_EMS, nc_EMS, c2v_comp_fact);
            [~,HD2] = max(-LLR_out,[], 2);
            HD2 = HD2'-1;
            [iters, dec_seq, success_dec] = ...
                presorted_MVSF_with_rinfrc(-LLR_out, max_iter, mul_mat, add_mat, div_mat,...
