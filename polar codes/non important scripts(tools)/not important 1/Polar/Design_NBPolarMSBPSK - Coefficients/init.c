...
//    for (k=1; k<nbRow; k++) decoder->APP[k] = decoder->APP[0] + k*nbCol;
    ...
    decoder->encoded_symbols=calloc((size_t)(code->n+1), sizeof(int *));
    decoder->encoded_symbols[0]=calloc((size_t)(code->n+1)*code->N, sizeof(int));
    for(k=1;k<code->n+1;k++)
        decoder->encoded_symbols[k]=decoder->encoded_symbols[0]+k*code->N;
}
...
void FreeDecoder (decoder_t *decoder)
{
    ...
    free(decoder->encoded_symbols[0]);
    free(decoder->encoded_symbols);
...
// multiply decimal values and output decimal
...
//divide decimal and output decimal
...
//        table->DIVDEC[i][j]=table->DECGF[table->DIVGF[i][j]];
        }
    }
...
void LoadCode (code_t *code, float SNR)
{
    ...
    int tmp;
    for(int i=0;i<N;i++){
        code->reliability_sequence[i]=i;//tmp;
    }

    printf("\nReliability Positions: \n");
    for(int i=0;i<N;i++)
        printf("%d ", code->reliability_sequence[i]);
    printf("\n");
