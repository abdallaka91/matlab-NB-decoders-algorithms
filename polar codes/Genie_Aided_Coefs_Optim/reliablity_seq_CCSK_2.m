...
elseif q==256
    CCSK_seq_str = ''; %GF256
elseif q==512
...
parfor prf=1:length(sigm)
    ...
    for i = 1 : n_obs
        ...
        [llr(:,:,n+1),gf(:,:,n+1), HD_L]=cond_prob_ccsk_1(y,sigm(prf),etaq,nm);
        ...
        for l2=n:-1:1
            for nc1=1:ncl(l2)
                for cl1=1:nkrnl(l2)
                    ...
                    L1=squeeze(llr(:,ii1,l2+1));
                    L2=squeeze(llr(:,ii2,l2+1));
                    gf1=squeeze(gf(:,ii1,l2+1));
                    gf2=squeeze(gf(:,ii2,l2+1));
                    u0=m1(ii1, l2);
                    u1=m1(ii2, l2);
                    %                     [Pr0, Pr1]= F1(L1, L2, add_mat,mul_mat, 1, u0);
                    [Pr0, Pr1, llr00, llr11, gf00, gf11]= F1_llr(L1, L2,gf1, gf2, add_mat,mul_mat, 1, u0, ofst, q);
                    gf(:,ii1,l2)=gf00;
                    gf(:,ii2,l2)=gf11;
                    llr(:,ii1,l2)=llr00;
                    llr(:,ii2,l2)=llr11;
    ...
    %%
    for l=1:n
        folderName = ['.\GF_' num2str(q) 'ccsk_reliab_seq\N' num2str(2^(n-l+1))];
